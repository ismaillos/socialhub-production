...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data | { error: string }>) {
  ...
  const apiKey = process.env.OPENAI_API_KEY; if(!apiKey){ return res.status(500).json({ error:"Missing OPENAI_API_KEY env var" }); }
  let langLabel = "French"; if(language==="EN") langLabel="English"; if(language==="AR") langLabel="Modern Standard Arabic";
  const typeLabel = contentType==="carousel" ? "carousel (sequence of image + text slides)" : contentType==="video" ? "short video script (Reel/TikTok)" : "image + text social post";
  const systemPrompt = `Type: ${typeLabel}
Language: ${langLabel}
Theme: ${theme}
Tone: ${tone}
Idea: "${idea}"
Return ONLY strict JSON:
{"hook":"short hook","body":"2-6 short sentences (or 3-6 slides)","cta":"call to action","visualDescription":"clean visual description for an image model","hashtags":["word1","word2","... up to 8"]}`.trim();
  try{
    const completion = await fetch("https://api.openai.com/v1/chat/completions",{
      method:"POST",
      headers:{ Authorization:`Bearer ${apiKey}`, "Content-Type":"application/json" },
      body: JSON.stringify({ model:"gpt-4o-mini", messages:[{ role:"system", content:"You are a top-tier social media content strategist." },{ role:"user", content: systemPrompt }], temperature:0.9 })
    });
    ...
    const hook = parsed.hook || ""; const body = parsed.body || ""; const cta = parsed.cta || "";
  ...
  }catch(err){ console.error("Error calling OpenAI:", err); return res.status(500).json({ error:"Internal server error" }); }
}
