...
def yt_callback(code: str = None, error: str = None):
    ...
    if not access_token: return JSONResponse({"status":"error","details": token_res}, status_code=400)
    ch = requests.get("https://www.googleapis.com/youtube/v3/channels?part=snippet&mine=true", headers={"Authorization": f"Bearer {access_token}"}).json()
    channel_id = None; channel_title = None
    if ch.get("items"): channel_id = ch["items"][0]["id"]; channel_title = ch["items"][0]["snippet"]["title"]
    with SessionLocal() as session:
        existing = session.query(Token).filter(Token.platform=="youtube").first() or Token(platform="youtube")
        existing.account_id = channel_id or "unknown"; existing.username = channel_title
        existing.access_token = access_token; existing.refresh_token = refresh_token
