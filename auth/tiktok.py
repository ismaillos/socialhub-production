...
def tiktok_callback(code: str = None, state: str = None, error: str = None):
    ...
    else: return JSONResponse({"status":"error","details": res}, status_code=400)
    ...
    with SessionLocal() as session:
        existing = session.query(Token).filter(Token.platform=="tiktok").first() or Token(platform="tiktok")
        existing.account_id = open_id or "unknown"; existing.username = username
        existing.access_token = at; existing.refresh_token = rt
